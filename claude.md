...
| `original_pdf_path` | string **nullable** (2026-09-01) | full relative path on `public` disk; **null** for a native-format document (see below) until/unless `convertToPdf()` is used |
| `native_path` | string nullable (2026-09-01) | the original Word/Excel/PowerPoint/... file, kept as-is with its real extension — set only for a native-format document, see `Document::isNativeFormat()` |
| `original_format` | string, default `'pdf'` (2026-09-01) | the real extension of what was uploaded (`pdf`, `docx`, `xlsx`, `jpg`, ...) — every pre-2026-09-01 row is `'pdf'` |
...
- **Password reset (self-service completion, M96, 2026-08-29)** needed the same treatment as
  logout for the same reason: the request that actually sets the new password
  (`ForgotPasswordController::reset()`) comes from a user who isn't logged in yet, so
  `LogMutation`'s `auth()->check()` is always `false` there too. `Illuminate\Auth\Events\PasswordReset`
  fires from inside `Password::reset()`'s success callback and still carries `$event->user`, so
  the same `AppServiceProvider` listener pattern records it as `auth.password-reset`.
...
| Search | `SearchController` | Public `GET /search?q=`; LIKE-based search across document titles, section names, rule set names/descriptions, folder names/descriptions; guests see `visibility = 'public'` docs and folders only; results capped at 50 docs + 20 sections + 20 rule sets + 20 folders; `->publishable()` scope hides pending/rejected. `document_type` and `state` query params filter independently of (and combinable with) `q`. **Browse-by-type pills (M92):** the search page shows a pill per document type with at least one visible document, count included, linking to `?document_type={key}` — an entry point into the same filter, not a separate search mode |
| **Government Orders (M92)** | **`DepartmentController::governmentOrders()`** | **`GET /departments/{level}/{department}/government-orders` — every `document_type = 'go'` document in a department, department-wide, paginated; same visibility rules (`publishable()`, `viewableBy()`, `publiclyVisible()` for guests) as every other document list; linked from a fourth card on the department show page** |
...
| **My Uploads** | **`Route::view()`, no controller (2026-08-31)** | **`GET /documents/my-uploads` — lists files still queued in this browser's IndexedDB from an abandoned upload batch, grouped by origin page, entirely client-rendered; linked from the header's pending-uploads badge** |
...
| POST | `/documents/chunk` | `documents.store-chunk` | Auth (M100) |
...
| GET | `/documents/{level}/{dept}/{section}/{doc}/native` | `documents.native` | Public* (M10x, 2026-09-01 — native-format doc's original file) |
...
| GET | `/documents/{level}/{dept}/{section}/divisions/{division}/{doc}/native` | `documents.divisions.native` | Public* (M10x) |
...
| GET | `/documents/{level}/{dept}/rules/{rule_set}/{doc}/native` | `documents.rules.native` | Public* (M10x) |
...
| GET | `/documents/{level}/{dept}/policy/{rule_set}/{doc}/native` | `documents.policy.native` | Public* (M10x) |
...
| GET | `/documents/{level}/{dept}/{section}/folders/{folder}/{doc}/native` | `documents.folders.native` | Public* (M10x) |
...
| GET | `/documents/{level}/{dept}/{section}/divisions/{division}/folders/{folder}/{doc}/native` | `documents.divisions.folders.native` | Public* (M10x) |
...
| GET | `/documents/my-uploads` | `documents.my-uploads.index` | Auth |
...
| POST | `/documents/{id}/convert-to-pdf` | `documents.convert-to-pdf` | Admin (controller check, M10x) — on-demand PDF for a native-format doc |
...
| GET | `/conversions/{quickConversion}/native` | `conversions.native` | Auth + owner (M10x) — native-format conversion's original file |
...
| GET | `/approvals/{id}/native` | `approvals.native` | Auth (M10x) — native-format doc's original file |
...
**Image → PDF normalization (M80, 2026-08-19; scope narrowed to images-only 2026-09-01 — see the native-format section below for Word/Excel/PowerPoint/...):** `original_pdf_path` and the whole OCR pipeline (`pdftoppm`, pdfminer, Docling) require actual PDF bytes, but an uploaded image (jpg/png/webp/gif/tiff/bmp/heic/heif) obviously isn't one. Before M80, `store()` just renamed whatever was uploaded to `.pdf` — an image upload became image bytes mislabeled `.pdf`, which failed loudly (and permanently — no working retry path) the moment the pipeline tried to read it. Fixed at the source: an image is first saved with a neutral `.upload` extension, run through `PdfConversionEngine::convertToPdf()` (`soffice --headless --convert-to pdf`, LibreOffice's Draw component), and only the resulting real PDF is kept as `{slug}_{timestamp}.pdf`; the original upload is deleted. Each conversion gets its own `-env:UserInstallation` profile dir under `storage/app/soffice-profile-*` (deleted after) — needed because the web user has no writable `$HOME`, and to keep concurrent uploads from sharing/locking one LibreOffice profile. Conversion failure → the temp upload is deleted and the request returns a 500 with a clear message. Timeout raised 120s → 240s (2026-09-01) — a large/complex source, or several conversions competing for CPU during a bulk upload, was hitting the old cap on genuine (not runaway) conversions. `QuickConversionController@store` had no image-handling branch at all before 2026-09-01 (an image uploaded via "New Conversion" silently failed) — now uses the same `PdfConversionEngine::convertToPdf()` call.

**Word/Excel/PowerPoint/ODT/RTF/TXT/CSV — kept native, no PDF round-trip (2026-09-01).** These formats are already selectable text or structured cells, with no scanned layout for OCR to do anything useful with — converting to PDF and then re-extracting the same text back out via pdfminer/Docling was pure overhead, and carried real fidelity risk (LibreOffice's headless Calc export can behave unexpectedly on a workbook with print-area/print-selection settings, though testing found no actual sheet loss on this LibreOffice version — see the evaluation note below). `Document::NATIVE_MARKITDOWN_MIMES` maps each accepted mime to its real extension; `DocumentController::createDocumentFromUpload()` branches on it — a native-format upload is stored as-is at `native_path` (its own extension, not `.pdf`), `original_pdf_path` stays `null`, `original_format` records the real extension. `ConvertDocumentToMarkdown::handleNative()` (and `ConvertQuickConversionToMarkdown`/`QuickConversionController@place`'s mirror-image equivalents) calls `PdfConversionEngine::convertNativeToMarkdown()` — `markitdown <file>` directly, no Docling, no OCR — and always lands in `'review'`, never `'ocr_pending'`. `Document::isNativeFormat()` / `QuickConversion::isNativeFormat()` gate this everywhere it matters: `convertOcr()` rejects a native document with an explanatory message instead of a generic 404; `BuildsZipDownload`, `ManagesDocumentFiles` (archive/restore/delete), move/copy (`relocateDocumentFiles`/`duplicateDocument`), and `ApprovalController::reclassify()` all carry `native_path` alongside `original_pdf_path` wherever the latter used to be assumed present.

A native document can still get a real PDF on request — `DocumentController::convertToPdf()` (`POST /documents/{id}/convert-to-pdf`, an explicit "Convert to PDF" button next to the viewer) runs `PdfConversionEngine::convertToPdf()` once and sets `original_pdf_path`, leaving `native_path`/`markdown_path`/status untouched — the document's Markdown was already extracted from the native file directly and doesn't change.

**Native-format viewing.** No PDF exists for these documents until `convertToPdf()` is used, so `documents/show.blade.php` (and `quick_conversions/show.blade.php`, and the approval-queue drawer) render the original file directly instead of the usual iframe: `public/js/native-preview.js` renders `.docx` via docx-preview.js and `.xlsx`/`.xls` via SheetJS (both loaded from `cdn.jsdelivr.net` — the only external script host this app's CSP allows, see `SecurityHeaders.php`; JSZip is a required dependency docx-preview's UMD build expects as a bare global, loaded first). Every other native format (`.doc`, `.ppt`/`.pptx`, `.odt`/`.ods`/`.odp`, `.rtf`/`.txt`/`.csv`) has no good self-hostable open-source in-browser renderer, so `native-preview.js` falls back to a plain download link rather than pretending to render them — `renderNativePreview()`'s `fallback()`. New routes serve the raw native file: `documents.native` (and its 4 division/folder/rule-set siblings, mirroring the existing `documents.pdf` family — `DocumentController::respondWithNative()`), `conversions.native`, `approvals.native`. The Compare & Verify modal and the "Run OCR Extraction" button are both native-aware — the OCR button is hidden entirely (`@unless($document->isNativeFormat())`) rather than shown and then rejected, since OCR genuinely doesn't apply here.

**Why LibreOffice for the PDF conversion that still happens (images, and on-demand `convertToPdf()`), not a PHP library (evaluated 2026-09-01, in response to a reported .xlsx upload failure and a fidelity concern — does converting lose sheets/formatting?):** Tested directly on this box — a 3-sheet workbook (including one with a restricted print area, since a source file's own Print Area/Selection setting was the most plausible way a real workbook could lose content) converts to a 3-page PDF with every sheet's content intact either way; see `tests/Feature/NonPdfUploadConversionTest.php`, which asserts on the actual `pdftotext` output of a real conversion, not a mock. PhpSpreadsheet/PHPWord's own PDF export (their `Pdf\Mpdf`/`Pdf\Dompdf` writers) is a simplified HTML-to-PDF pass, not a real layout engine — in practice it reproduces complex formatting (merged cells, embedded charts, mixed fonts) *less* faithfully than LibreOffice's actual rendering, which is the same renderer the desktop application uses. Client-side conversion (mammoth.js for docx, sheetjs for xlsx) was also considered and rejected for the *conversion* role specifically: neither produces a real PDF (HTML output only), neither covers the legacy binary formats (`.doc`/`.xls`/`.ppt`) or `.odt`/`.rtf`, and the server-side LibreOffice step would still be needed as a fallback regardless. (SheetJS and docx-preview are still used client-side, just for *viewing* a native file, not for producing a PDF from it — a materially different job with a much lower fidelity bar.)
...
- `<x-share-button :title="...">` (`components/share-button.blade.php`, 2026-09-02) — the WhatsApp/X/copy-link dropdown, extracted from the document show page (which had it inline first) so department/section/division/folder show pages could reuse it verbatim rather than each carrying its own copy. Purely client-side Alpine, always shares `url()->current()` — no controller action, no route, nothing server-side to add when using it. Visible to every visitor regardless of the container's own visibility, same reasoning as the document page always showed it: the button doesn't expose anything a viewer looking at the page couldn't already copy from the address bar. Present on `documents/show.blade.php`, `department/show.blade.php`, `sections/show.blade.php`, `divisions/show.blade.php`, `folders/show.blade.php` — pass the container's own `name`/`title` as `:title`.
...
- **Activity log gaps closed (M96, 2026-08-29).** Two gaps in the audit trail, both surfaced by an
  admin reading the activity log and not recognizing what they saw. First, `ACTION_LABELS` was
  missing entries for several real, already-logged routes — `admin.users.send-password-reset`,
  `admin.users.password-reset-link`, `documents.verify`, `documents.move`, `documents.copy` — so
  they showed up as raw route names instead of the short labels every other action gets; added all
  five (see the `activity_logs` schema section above for the general "keep this in sync" rule this
  falls under). Second, and less obvious: a self-service password reset was never recorded at all,
  because the request that sets the new password isn't authenticated yet — see the `PasswordReset`
  listener note above.
...
- **Client-side PDF split for large uploads (M100, 2026-08-31).** The app's own 300 MB cap is moot for a PDF over ~100 MB — the production Cloudflare Tunnel rejects it at the edge before Apache/PHP ever sees it (see DEPLOY.md's "Cloudflare's own edge upload cap"). `public/js/resilient-upload.js`'s `uploadFile()` — the one call every upload modal's submit loop makes now — splits a PDF over 95 MB into ≤90 MB pieces client-side (pdf-lib via CDN) and POSTs them to `documents.store-chunk` instead of `documents.store`. Page-count-per-piece is estimated from the file's average bytes/page, not packed exactly (would mean re-serializing a growing trial chunk per page — O(n²)); real scanned PDFs have uniform enough page sizes that this stays close to the target. Server side, `DocumentController::storeChunk()` (`StoreDocumentChunkRequest`, extending `StoreDocumentRequest` so it reuses `authorize()`/every non-file rule) stores each piece under `storage/app/private/pdf-chunk-uploads/{upload_id}/`; the last piece triggers `pdfunite` (poppler-utils, already on the box) to reassemble the original PDF, then a synthetic `UploadedFile` pointed at the merged file runs through `createDocumentFromUpload()` — `store()`'s actual document-creation logic, extracted into its own method precisely so both paths share it — exactly like a normal single-file upload. One `Document` row results either way; the split is invisible once upload finishes. `PruneChunkedUpload`, delayed 24h from the first chunk, cleans up an abandoned chunk directory (same pattern as `PruneQuickConversion`). Non-PDF files aren't split — a 95+ MB Word/Excel/image upload still hits the tunnel cap unchanged, out of scope for this feature. `uploadFile()` catches a split failure (a 0-page or otherwise unreadable PDF) and returns the normal `{ok:false,…}` shape instead of throwing — the submit loops destructure its result with no `try`/`catch` of their own, so an uncaught throw here would have crashed the whole batch loop, not just failed the one file. `upload_id` is a client-picked UUID, not a secret, so `storeChunk()` writes a `.owner` marker (the uploading user's id) alongside chunk 0 and checks it on every later chunk — otherwise a second authenticated user who observed or guessed someone else's `upload_id` could append chunks into, or trigger assembly of, their in-progress upload. Covered by `tests/Feature/ChunkUploadOwnershipTest.php`.

- **Pending-uploads indicator (2026-08-31).** Uploading is client-driven — the browser reads local files and posts them one at a time from whichever section/division/folder page the modal is open on. Navigating away mid-batch tears down that page's JS entirely, so the upload stops (files already sent are safe; anything not yet sent just sits in `ResilientUpload`'s IndexedDB queue, unable to resume until the same page is opened again — the `offerResume()` prompt already handled that half). What was missing was any way to *notice* an abandoned batch from somewhere else. `Queue`'s constructor now takes an optional `{url, label}` — the page and human-readable name to send someone back to — stored on each queued row alongside the file; `ResilientUpload.allQueued()` reads every row across every page's queue (not just one page's own, like `Queue.all()`), grouped by that same `{url, label}`. `components/header.blade.php` checks `allQueued()` on every authenticated page load and shows a small badge (queued-file count) if anything's pending; it links to `documents.my-uploads.index` (`documents/my-uploads.blade.php`, a bare `Route::view()` — the data is entirely client-side IndexedDB, no controller or DB table involved), which lists every pending file grouped by its origin page with a "Resume here" link. Filenames and folder/section/division names are user-controlled and rendered via `innerHTML` for the grouping — both are run through a local `escapeHtml()` first. Covered by `tests/Feature/MyUploadsPageTest.php`.
  A row queued before this feature shipped has no `pageUrl`/`pageLabel` (the fields didn't exist yet) — it still lists, under "Unknown destination…", with no "Resume here" link since there's nowhere to send it back to. `ResilientUpload.removeQueued(id)` (2026-09-01) deletes a queued row by id regardless of which page's `key` it belongs to, so `my-uploads.blade.php` can offer Discard (per file and per group) — the only way to clear one of these otherwise-stuck legacy rows.

- **A long queue-row error message used to crush the title input it was reported next to (2026-09-02).** Each queued file's `statusBadge` span is `flex-shrink-0` — fine for a short static label ("Pending"/"Uploading…"/"✓ Done"), but `setRowStatus()` used to write the full dynamic error message straight into it too (e.g. "✗ Title must contain at least one letter — a filename or ID number alone makes a poor, unreadable document URL", the message a purely-numeric filename's auto-filled title always triggers). A `flex-shrink-0` element refuses to shrink, so a message that long forced the badge to its full unwrapped width and squeezed the sibling `meta` column — which holds the very `titleInput` the user needs to fix the title — down to a sliver, hiding it entirely. Fixed in all four upload views (`sections/show.blade.php`, `divisions/show.blade.php`, `folders/show.blade.php`, `documents/bulk-upload.blade.php`, each with its own duplicated queue-row JS, no shared component): `statusBadge` now always shows a short fixed label per state (adding `error: '✗ Error'`), and the full message goes into a new `errorLine` paragraph appended inside `meta` itself, right under `titleInput`/`sizeLine` — it wraps normally there instead of blowing out the row, and the input stays fully visible and usable to actually fix the title.
- **Silent disk-write failures now get logged (2026-09-01).** `config/filesystems.php`'s `public`/`local`/`s3` disks are `'throw' => false` (an upload failure returns `false` for `DocumentController`'s own `if (!$storedPath)` checks to handle, rather than throwing) but were also `'report' => false`, which additionally suppressed Laravel's automatic `report()` call on the real Flysystem exception — so a "File could not be saved" a user hit left *nothing* in `storage/logs/laravel.log`, not even the real reason (permission denied, disk full, a bad path). Flipped to `'report' => true`; behavior is unchanged (still returns `false`, not a thrown exception) but the real failure now gets logged. `DocumentController::createDocumentFromUpload()` and `PolicyDocumentController::store()` additionally log their own `Log::error()` with request context (user, vault dir, filename, size) right where the `!$storedPath` check already was, since the automatic Flysystem report has no request context of its own. Covered by `tests/Feature/UploadFailureLoggingTest.php` (forces a real permission-denied write against `Storage::fake()`'s disposable disk).
...
- **Never nest a `<form>` inside another `<form>`** — a common shape here is a page's main "Save" form with a "Delete" button/form in its footer. Nested `<form>` tags are invalid HTML; the browser's HTML5 parser drops the inner `<form>` open tag entirely (attributes and all) but keeps its children — including any `@method('DELETE')` hidden input — as children of the *outer* form, then closes the *outer* form early on the inner tag's `</form>`. Result: the outer form gets two `_method` hidden inputs (its own, plus the dropped inner one), duplicate field values resolve to the **last** one server-side, and everything after the premature close (including the outer form's own submit button) ends up outside any form at all. This is exactly what happened in M89 (2026-08-22, see `summary.md`) — editing a Section's visibility and clicking "Save Changes" submitted as a DELETE instead, deleting the section with no confirm dialog (the `onsubmit` confirm was on the discarded inner tag). Fix pattern: make the previously-nested form a sibling `<form>` elsewhere in the markup, and have the outer form's button reference it via the HTML5 `form="<id>"` attribute instead of physical nesting — see `sections/edit.blade.php`, `department/edit.blade.php`, `admin/users/edit.blade.php` for the corrected shape. When adding a new field to any existing edit page, glance at the whole file's form structure while you're in there, not just the field you're adding.
- **Delete confirmations use SweetAlert2** (already loaded globally in `head.blade.php`), not the native `confirm()` — attach the confirm to the button's `onclick`, not a form's `onsubmit` (an `onsubmit` handler can be silently discarded by the nested-form parsing quirk above; an `onclick` on the visible button cannot).
