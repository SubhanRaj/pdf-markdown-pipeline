...
- Mail: `app/Mail/AccountOnboarding.php`, `app/Mail/LoginOtp.php`, both `->view()`-based (not Markdown mail), both extending `resources/views/emails/layout.blade.php` (shared inline-CSS wrapper — email clients don't support Tailwind). Transport-agnostic by design: `.env`'s `MAIL_MAILER` picks `resend` (needs `RESEND_API_KEY`, `composer.json` already has `symfony/resend-mailer`) or `smtp` (NIC/Gmail/etc, `MAIL_HOST`/`PORT`/`USERNAME`/`PASSWORD`) — neither Mailable references a transport directly, both just call through `Mail::to()->send()`.
