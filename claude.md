...
**Icon font is subset, not the full Tabler set (2026-07-26, refreshed 2026-07-28)** — `public/vendor/tabler-icons/tabler-icons.min.css`/`fonts/tabler-icons-subset.woff2` contain only the **117 `ti-*` classes this app actually uses** (subset via `fonttools`/`pyftsubset` from the full ~5,900-icon set), not the full library. This dropped the font from ~892KB (woff2) + 247KB (css) down to ~13KB + ~5KB — the app was only using ~2% of the bundled icons. Same class names (`<i class="ti ti-xxx">`), zero Blade template changes from the subsetting itself. Rebuilt 2026-07-28 to add `ti-file-zip`, `ti-lock-check`, `ti-mail-exclamation` (the bulk-ZIP-download feature added `ti-file-zip` to the header buttons but nobody added its glyph to the subset — it silently rendered nothing, only caught when checking why the mobile download button "wasn't working"; the other two were already unused-but-present gaps found while auditing). Rebuilds should union the previous subset's classes with the newly-needed ones — never regenerate from a fresh grep alone, since a class can be referenced dynamically (string-built in Blade) and not show up in a static search.
