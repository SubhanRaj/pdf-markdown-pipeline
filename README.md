...
| Frontend / UI | Blade Templates, Tailwind CSS v4 (Play CDN + `typography` plugin), Alpine.js (self-hosted, client-side interactivity/polling), Parsedown, `marked.js` (CDN, client-side Markdown preview), `Grid.js` (CDN, sortable/searchable tables in the structure panel), `Air Datepicker` (CDN, calendar date pickers on the Policy Period create/edit form) — no Node, no npm, no build step |
