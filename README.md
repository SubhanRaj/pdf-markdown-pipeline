...
- **Policy browsing (2026-07-26)** — `/policy` is a 2-card landing page ("Uttar Pradesh Policy" / "Other States' Policy") rather than a flat state-grouped list. `/policy/state/{state}` is one page shared by both the UP card and every other-state card (filtered by state, no UP-specific code path) — shows each of that state's containers with its current period featured and previous years in a grid below. `/policy/other-states` lists every state/UT as a card (0-count states still clickable, so an admin has somewhere to land before uploading a first container for that state). Pure view/routing change — no new data model, see [POLICY_PERIODS.md](POLICY_PERIODS.md) §4.
...
| Frontend / UI | Blade Templates, Tailwind CSS v4 (Play CDN + `typography` plugin), Alpine.js (self-hosted, client-side interactivity/polling), Parsedown, `marked.js` (CDN, client-side Markdown preview), `Grid.js` (CDN, sortable/searchable tables in the structure panel), `Air Datepicker` (CDN, calendar date pickers on the Policy Period create/edit form), `@svg-maps/india` (CDN via jsDelivr, CC-BY-4.0, client-side state outline icons on the Policy pages) — no Node, no npm, no build step |
...
- Rule set upload flow: two independent modals — "Upload Document" (always enabled; only the root `rule`/`policy` type is capped at one per rule set, disabled in the type dropdown once taken, so supplementary docs like "Other" checklists stay uploadable afterward) and "Upload Amendment" (disabled until a rule doc exists); amendment modal auto-selects the parent if only one root rule doc is present; rule set cascade delete soft-deletes all documents with audit entries before removing the rule set; Edit button locked on rule docs that already have amendments
...
- **Session hardened** — `SESSION_ENCRYPT=true`, `SESSION_SAME_SITE=strict` applied; `SESSION_SECURE_COOKIE` documented (must be `true` on HTTPS SDC deployment). *(2026-07-26: `SESSION_EXPIRE_ON_CLOSE` and the remember-me checkbox were later reintroduced — see below, this deployment is personal-PC not shared-workstation.)*
...
|---|---|---|---|
| Admin | `redacted-personal-email@example.com` | `REDACTED-PASSWORD` | Full access (`*`) — primary dev account |
| Admin (demo) | `admin.demo@excise.up.gov.in` | `REDACTED-PASSWORD` | Full access (`*`) — Deputy Commissioner persona |
...
**Completed (2026-07-25 — Kruti Dev content-heuristic detection, dynamic OG/SEO, sitemap, Blade double-escape fix, self-hosted Tabler Icons):**
- Legacy-font (Kruti Dev/Chanakya/etc.) detection no longer depends solely on the embedded font's *name* — a content-based fallback (checks for the specific Latin-1 codepoints these fonts remap matras onto, with zero real Devanagari Unicode present) catches PDFs whose export tool anonymized/subset the font name. Caught 3 silently-garbled documents from the same day's bulk import; all 190 imported documents reset and reprocessed with the fix.
- `<x-head>`/`<x-layout>` now emit real per-page `<meta name="description">`, Open Graph, and `twitter:card` tags — document pages describe the actual document and use a real page-1 PDF thumbnail as `og:image` (rasterized on first request, cached); rule-set pages describe the department/rule-set/document-count. WhatsApp/X/copy-link share buttons added to document pages.
