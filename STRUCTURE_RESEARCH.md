...
**Date:** 2026-07-15
**Status:** Phase 1 (structure detection only, no merge into rendered Markdown) implemented —
see `summary.md` M32. This file records what was tried and found during evaluation, so the
reasoning trail isn't lost — same purpose `OCR_RESEARCH.md` serves for the character-accuracy
side of this pipeline. Continues from the three-pass idea proposed in chat and originally
recorded in `STRUCTURE_HANDOFF.md` — since retired now that this file supersedes it; see git
history if the original handoff text is needed.
...
policy batch (`~/Excise policies of states/` on the Ubuntu AIO box):
...
PaddleOCR or Surya directly. This matters less than it sounds: Docling's own OCR text is only
used to read scanned regions well enough to detect their *structure* (headings/tables); the
actual character-accurate text still comes from whichever engine (Tesseract/EasyOCR/PaddleOCR/
...
**This round (M32, Phase 1) ships the structure-detection half of Strategy B only** — the
compact structure map is produced and shown to reviewers, but the actual bounding-box merge into
rendered Markdown is deferred (Phase 2) until real structure output has been reviewed against
enough real documents in the UI to confirm the heuristic-only table/heading detection already in
`pdf_structure_extractor.py` is actually insufficient in practice, rather than building the more
complex merge algorithm blind.

## Open follow-ups, not part of this round

- **Geometric merge (Phase 2)** — align OCR word-boxes (Tesseract hOCR bboxes, EasyOCR/PaddleOCR
  bboxes already emitted by `pdf_structure_extractor.py`) into Docling's `tables[].data.table_cells[]`
  / `texts[]` bboxes, replacing the current heuristic-only row/column clustering for documents
  that have a structure map available.
