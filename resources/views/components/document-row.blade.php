...
@php
    ...
    // Only offered when the caller eager-loaded 'amendments' (root+amendments tree pages) —
    // accessing the relation on a page that didn't would trigger an N+1 query per row.
    $hasAmendments = ! $isAmendment && $doc->relationLoaded('amendments') && $doc->amendments->isNotEmpty();
    ...
    $effectiveDate = $effYear
        ? ($effDay && $effMonth ? "{$effDay} {$monthNames[$effMonth]} {$effYear}" : ($effMonth ? "{$monthNames[$effMonth]} {$effYear}" : (string) $effYear))
        : null;
@endphp

{{-- Whole row is a clickable link (stretched <a> at the end, z-0) — icon actions above it
     stay individually clickable via z-10, same pattern across every document listing. --}}
<div {{ $attributes->class([
        'relative flex items-start gap-3 px-5 py-3.5 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors group',
        'bg-amber-50/30 dark:bg-amber-900/5 border-l-2 border-amber-200 dark:border-amber-700 ml-6 pl-4' => $isAmendment,
    ]) }}>
    ...
    <div class="relative z-10 flex items-center gap-1 flex-shrink-0 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
        ...
        @auth @if($destroyUrl && auth()->user()->isAdmin())
        <button type="button"
    ...
    <a href="{{ $url }}" class="absolute inset-0" aria-label="Open {{ $doc->title }}"></a>
</div>
