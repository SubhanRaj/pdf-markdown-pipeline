@php
    ...
    $updateUrl = $isDivisionFolder
        ? route('departments.sections.divisions.folders.update', [$department->levelAlias(), $department, $section, $division, $folder])
        : route('departments.sections.folders.update', [$department->levelAlias(), $department, $section, $folder]);
...
<x-breadcrumb :items="[
    ...
    ['name' => $folder->name,                'url' => $backUrl],
    ['name' => 'Edit',                       'url' => null],
]" />

<form id="folderForm" method="POST" action="{{ $updateUrl }}" novalidate class="max-w-2xl">
    @csrf
    @method('PATCH')
        ...
        <div class="px-6 py-5">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" value="{{ $folder->slug }}" disabled
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="field-input resize-none @error('description') field-error @enderror">{{ old('description', $folder->description) }}</textarea>
                ...
                <div>
                    ...
                    <div class="flex gap-3 mt-1">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="visibility" value="public" class="text-cyan-600 focus:ring-cyan-500" @checked($folder->visibility === 'public')>
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="visibility" value="authenticated" class="text-cyan-600 focus:ring-cyan-500" @checked($folder->visibility === 'authenticated')>
                ...
                @if(auth()->user()->isAdmin() || auth()->user()->hasPrivilege('department.head') || auth()->user()->hasPrivilege('section.head'))
                <div class="pt-4 border-t border-slate-200 dark:border-slate-700">
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        <span class="text-sm font-medium text-slate-700 dark:text-slate-300">Require approval for all uploads to this folder</span>
                    </label>
                    <p class="mt-1 ml-7 text-xs text-slate-500 dark:text-slate-400">When enabled, any document uploaded to this folder is held as "Pending Approval" until an approver reviews it.</p>
                </div>
                @endif
